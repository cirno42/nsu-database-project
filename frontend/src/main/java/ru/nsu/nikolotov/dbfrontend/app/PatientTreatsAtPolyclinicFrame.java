...
public class PatientTreatsAtPolyclinicFrame {
    private static final int FRAME_WIDTH = 600;
    private static final int FRAME_HEIGHT = 600;


    private final JFrame frame = new JFrame("Patient treats at polyclinic");
    ...
    private List<PatientTreatsInPolyclinicEntity> entities = new ArrayList<>();
    ...
    public PatientTreatsAtPolyclinicFrame() {
...
//        updateEntityButton.addActionListener(l -> System.out.println(idTextField.getText()));
        ...
        deleteEntityButton.addActionListener(l -> {
            PatientAPI.finishPatientTreatmentInPolyclinic(entities.get(listIndex).getPatientId());
    ...
    private void getAllWards() {
        entities = PatientAPI.getPatientsWhoTreatsInPolyclinic();
    }
    ...
    private void fillGotoCombobox() {
        ...
        for (int i = 0; i < entities.size(); i++) {
            idAndNames[i] = entities.get(i).getDoctorName() + " " + entities.get(i).getPatientName();
